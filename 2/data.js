const input = `Game 1: 1 green, 2 red, 6 blue; 4 red, 1 green, 3 blue; 7 blue, 5 green; 6 blue, 2 red, 1 green
...
const games = input.split("\n").map((string) => {
  const [gameString, setsString] = string.split(": ");
  const id = gameString.replace("Game ", "");
  const draws = setsString.split("; ");
  const results = draws.reduce((results, draw) => {
    const result = draw.split(", ").reduce(
      (drawResult, drawElement) => {
        const [numberString, colour] = drawElement.split(" ");
        const n = Number(numberString);
        switch (colour) {
          case "red":
            return { ...drawResult, red: n };
          case "blue":
            return { ...drawResult, blue: n };
          case "green":
            return { ...drawResult, green: n };
          default:
            return drawResult;
        }
      },
